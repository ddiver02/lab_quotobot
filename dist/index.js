...
const ai = (0, genkit_1.genkit)({
    plugins: [(0, google_genai_1.googleAI)({ apiKey: process.env.GOOGLE_API_KEY || '' })],
...
const QuoteMatchSchema = genkit_1.z.object({
    ...
    quote: genkit_1.z.object({
        ...
        quote: genkit_1.z.string(),
        emotion: genkit_1.z.array(genkit_1.z.string()),
    }),
});
// ─────────────────────────────────────────────
// Flow: 입력 문장에 어울리는 인용구 매칭
// ─────────────────────────────────────────────
exports.quoteMatchFlow = ai.defineFlow({
    name: 'quoteMatchFlow',
    ...
    if (typeof rawText !== 'string')
        rawText = String(resp.output_text ?? '');
    if (!rawText)
...
app.post('/api/quote', async (req, res) => {
    try {
        ...
        const out = await (0, exports.quoteMatchFlow)(input);
        return res.json(out);
    }
    catch (e) {
